...
function events_meta () {
	...
	$eventslocation = $custom["events_location"][0]; 
    ...
    <td><label>Recur Schedule</label></td><td><select name="events_recur" class="select" style="width:100%;" > 
            ...
            <option value="monthly" <?php if($eventsrecur == 'monthly'){ echo ' selected'; } ?> >Monthly</option>	
            <option value="daily" <?php if($eventsrecur == 'daily'){ echo ' selected'; } ?> >Daily</option>			
    	</select></td><td>&nbsp;</td><td>&nbsp;</td>
    </tr>
    <tr>
    <td ><label>Location</label></td><td colspan="4"><input name="events_location" class="large-text" value="<?php echo $eventslocation; ?>" /></td>
    </tr>
...
function events_create() {
    add_meta_box('events_meta', 'Event Info', 'events_meta', 'events');
}
...
function events_custom_columns($column){
	...
	case "events_start_date":
		// - show dates -
		$eventsstartdate = date('m/d/Y', strtotime($custom["events_start_date"][0]));
	...
	case "events_end_date":
		// - show dates -
		$eventsenddate = date('m/d/Y', strtotime($custom["events_end_date"][0]));
		echo $eventsenddate;
	break;
...
function getevents_func( $atts, $content = null ) {
	...
	if($debugevents){
		$output = '<p>location = '.$a['location'].', category = '.$a['category'].'</p>';
	}else{
		$output = '';
	}
  ...
  foreach($eventlist as $event){
	  ...
	  if($start_date != $end_date){ 
		  ...
		  if($recur == 'monthly'){
			  ...
			  //if($start_time != ''){ $fulldate .= ', '.$start_time; }
			  //if($end_time != '' && $end_time != $start_time){ $fulldate .= ' &ndash; '.$end_time; }
		  }
		  if($recur == 'daily'){
			  $fulldate = 'Every day, '.date('M j, Y', $start_date).' &ndash; '.date('M j, Y', $end_date);
			  if($start_time != ''){ $fulldate .= ', '.date('g:i a', $start_time); }
	  ...
	  $output .= '<div id="event'.$u.'" >';
		...
		$output .= '<p><a href="'.$link.'" class="title"><strong>'.$title.'</strong></a><br><span class="fulldate">'.$location.'</span><br><span class="fulldate">'.$fulldate.'</span>';
		if($new_date!=$start_date && $new_date!=''){ $output .= '<br><em><strong>Next occurrence:</strong> <span class="next">'.date('M j, Y',$new_date).'</span></em>'; }
	  $output .= '</p></div>';
	  $u++;
  }
